...
{
    /// <summary>
    /// Bordy entry MonoBehaviour. Builds an accordion-style category list of TT.* API
    /// buttons at runtime, wires each one to a handler, and pipes results into the log
    /// panel. The UI itself is generated by <see cref="EditorTools.BordySceneBuilder"/>.
    /// Bordy 入口 MonoBehaviour。运行时构建可折叠的 TT.* 接口按钮列表，把每个按钮接到对应
    /// 调用，并把回调写到日志面板。UI 本身由 <see cref="EditorTools.BordySceneBuilder"/> 生成。
    /// </summary>
